...
.animate-float {
  animation: float linear infinite;
            ...
            <div>
                {/* Header */}
                <section className='relative min-h-screen flex items-center justify-center bg-gradient-to-b from-[#7dbea8] to-[#98d8dc] p-4 overflow-hidden'>
                    ...
                    <ScrollRevealElements
                        className='relative z-10 text-center mb-40'
                        staggerAmount={0.5}
                    >
                        <motion.div className='flex justify-center mb-4'>
                            <Player autoplay loop src="/lottie-assets/Recycle-Process-Animetion/animations/175a9a37-546c-4c9a-9f82-4c1bf51fb1ac.json" style={{ width: '190px', height: '190px' }} />
                        </motion.div>
                        <motion.h1 className='relative z-10 text-5xl md:text-7xl font-extrabold text-white uppercase tracking-wider'>
                        ...
                        <motion.div className="flex flex-col items-center">
                            ...
                            <div onMouseEnter={() => setActiveNode('center')} onMouseLeave={() => setActiveNode(null)} className={`relative group rounded-xl shadow-lg transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-2xl ${activeNode === 'center' ? 'ring-2 ring-[#10b981]' : 'ring-1 ring-gray-200'}`}>
                                ...
                                <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl">
                                    <p className="text-white font-bold text-2xl">Polluted Waterbody</p>
                                </div>
                            </div>
                        </motion.div>
                ...
                <section className="bg-[#e2e9e4] pt-16 pb-20 px-4">
                    <div className="container mx-auto">
                        ...
                        <ScrollRevealElements
                            ...
                            staggerAmount={0.5}
                        >
                            <motion.h2 className="text-6xl md:text-6xl font-bold text-slate-800">
                                Case Study: <span className="text-[#10b981]">{caseStudy.title}</span>
                            </motion.h2>
                            <motion.p className="text-slate-500 mt-4 max-w-3xl mx-auto leading-relaxed">{caseStudy.intro}</motion.p>
                        </ScrollRevealElements>
                        ...
                        >
                                ...
                                {hoveredStage === 'before' ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 h-full w-full">
                                        {caseStudy.beforeImages.map((src, idx) => (
                                            <div key={idx} className="h-full w-full bg-cover bg-center" style={{ backgroundImage: `url(${src})` }} />
                                        ))}
                                    </div>
                                ) : (
                                    <div className="relative w-full h-full bg-red-800 p-8">
                                        <img src={caseStudy.beforeImages[0]} alt="Polluted water" className="absolute inset-0 w-full h-full object-cover opacity-20" />
                                        <h3 className="relative z-10 text-3xl font-bold text-white ">Before</h3>
                                    </div>
                                )}
                            </div>
                            {/* middle */}
                            <div className={`relative flex flex-col p-8 transition-all duration-500 ease-in-out ${hoveredStage === null ? 'lg:w-[50%]' : 'lg:w-[25%]'} flex-grow bg-slate-100 text-slate-800 justify-center items-start`}>
                                <div className="w-full text-center">
                                    <h3 className="text-3xl font-bold mb-6 text-[#10b981]">Restoration Journey</h3>
                            ...
                            >
                                {hoveredStage === 'after' ? (
                                    ...
                                    <div className="relative w-full h-full bg-green-800 p-8 flex justify-end items-start">
                                        <img src={caseStudy.afterImages[0]} alt="Restored water" className="absolute inset-0 w-full h-full object-cover opacity-20" />
                                        <h3 className="relative z-10 text-3xl font-bold text-white">After</h3>
                                    </div>
                                )}
                            </div>
                        </motion.div>
                ...
                <section className="bg-[#f6f6f6] pt-20 pb-20 px-4">
                    <div className="container mx-auto">
                        ...
                        <motion.div
                            ...
                            transition={{ duration: 0.7, ease: "easeOut" }}
                        >
                            <h2 className="text-6xl md:text-6xl font-bold text-slate-800">
                                Other <span className="text-[#10b981]">Projects</span>
                            </h2>
                        </motion.div>

                        <motion.div
                            className="flex flex-col lg:flex-row rounded-2xl overflow-hidden shadow-2xl bg-gray-100 min-h-[400px]"
                            initial={{ opacity: 0, y: 50 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            transition={{ duration: 0.8, ease: "easeOut" }}
                        >
                            {['laxmi', 'csjm'].map(key => (
                                <div
                                    ...
                                    className={`relative transition-all duration-500 ease-in-out flex-grow cursor-pointer ${hoveredProject === key ? 'lg:w-2/3' : 'lg:w-1/3'}`}
                                >
                                    <img
                                    ...
                                    />
                                    <div className={`absolute inset-0 transition-colors duration-300 ${hoveredProject && hoveredProject !== key ? 'bg-black/60' : 'bg-transparent'}`} />
                                    <div className="relative z-10 p-8 flex flex-col justify-end text-white h-full bg-gradient-to-t from-black/70 to-transparent">
                                        <h3 className="text-2xl font-bold">{projectsData[key].title}</h3>
                                        <div className={`transition-all duration-500 overflow-hidden ${hoveredProject === key ? 'max-h-40 mt-4' : 'max-h-0'}`}>
                                            <p className="text-slate-200">{projectsData[key].description}</p>
                                            {projectsData[key].details && (
                                                <p className="mt-2 font-semibold">{projectsData[key].details}</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
